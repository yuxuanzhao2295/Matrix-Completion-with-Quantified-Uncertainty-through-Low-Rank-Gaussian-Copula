...
# search sequence
rankLRGC = c(5,5)
ratiosoftImpute = exp(seq(from=log(1),to=log(1/100),length=9))[c(5,3)]

errReliability_ord = array(0, dim = c(nrep,nquantiles,2,2))
for (i in 1:nrep){
  for (j in 1:2){
    ...
    errReliability_ord[i,,j,1] = mae_by_reliability(r = r[loc], xtrue = Xtrue[loc], ximp = est$Ximp[loc])
    ...
    errReliability_ord[i,,j,2] = mae_by_reliability(r = -r, xtrue = Xtrue[loc], ximp = ximp[loc])
  }
  print(paste('finish iteration: ',i))
}
save(errReliability_ord, file = 'ResReliability_LRGC_softImpute_SimOrdinal.RData')

err = apply(errReliability_ord, c(2,3,4), mean)
