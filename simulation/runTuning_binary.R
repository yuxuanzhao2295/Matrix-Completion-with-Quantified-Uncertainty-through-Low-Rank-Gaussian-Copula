...
resTuning_bin = array(0, dim = c(nrep,9,2,2,3), 
...
for (i in 1:nrep){
  for (j in 1:2){
    ...
    for (s in 1:9){
      ...
      b = Sys.time()
      resTuning_bin[i,s,1,j,1] = cal_mae(est$Ximp, X_obs, Xtrue)
      resTuning_bin[i,s,2,j,1] = difftime(b,a,units = 'secs')
      
      # PPCA
      a = Sys.time()
      est = pca(X_obs, method = 'ppca', nPcs = rankPPCA[s])
      b = Sys.time()
      resTuning_bin[i,s,1,j,2] = cal_mae(trunc.rating(est@completeObs,xmin = 0,xmax = 1), X_obs, Xtrue)
      resTuning_bin[i,s,2,j,2] = difftime(b,a,units = 'secs')
      ...
      b = Sys.time()
      resTuning_bin[i,s,1,j,3] = cal_mae(trunc.rating(ximp,xmin = 0,xmax = 1), X_obs, Xtrue)
      resTuning_bin[i,s,2,j,3] = difftime(b,a,units = 'secs')
      warm=fiti
    }
  }
...
save(resTuning_bin, file = 'ResTuning_LRGC_PPCA_softImpute_SimBinary.RData')
err = apply(resTuning_bin, c(2,3,4,5), mean)
